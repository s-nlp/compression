...
def tokenize_tracking_span(tokenizer, text, spans):
    ...
    for i in range(2, len(full_toks)):
        ...
        partial_txt_len = len(
            tokenizer.decode(full_toks[:i], clean_up_tokenization_spaces=False)
        )
...
def tokenize_and_encode_spans_wic(tokenizer, text, spans):
    """
    Encode while tracking what tokens spans (char idxs) get mapped to
    Strategy: split input around span, encode left of span, the span,
        and then recursively apply to remaning text + spans
    We assume spans are
        - inclusive on start and end
        - non-overlapping
    ...
    text_before_span = text[: span[0]]
    # get the text inside the span
    text_inside_span = text[span[0] : span[1]]
    # get the text after the span
    text_after_span = text[span[1] + 1 :]

    # encode the text before the span
    encoded_text_before_span = tokenizer.encode(
        text_before_span, add_special_tokens=True
    )
    # encode the text inside the span
    encoded_text_inside_span = tokenizer.encode(
        text_inside_span, add_special_tokens=False
    )
    # encode the text after the span
    encoded_text_after_span = tokenizer.encode(text_after_span, add_special_tokens=False)

    # get the length of the encoded text before the span
    encoded_text_before_span_length = len(encoded_text_before_span)
    # get the length of the encoded text inside the span
    encoded_text_inside_span_length = len(encoded_text_inside_span)
    # get the length of the encoded text after the span
    encoded_text_after_span_length = len(encoded_text_after_span)

    # get the start position of the span in the encoded text
    encoded_span_start = encoded_text_before_span_length
    ...
    return encoded_text, [(encoded_span_start, encoded_span_end)]
...
def superglue_convert_examples_to_features(
    ...
    if is_torch_dataset and isinstance(examples, torch.utils.data.Dataset):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if isinstance(example, SpanClassificationExample):
            ...
            if example.spans_b is not None:
                if task == "wsc":
                    inputs_b, span_locs_b = tokenize_tracking_span(
                        tokenizer, example.text_b, example.spans_b
                    )
                else:
                    inputs_b, span_locs_b = tokenize_and_encode_spans_wic(
                        tokenizer, example.text_b, example.spans_b
                    )
                num_non_special_tokens = (
                    len(inputs_a["input_ids"]) + len(inputs_b["input_ids"]) - 4
                )
                ...
                num_joiner_specials = (
                    len(inputs["input_ids"]) - num_non_special_tokens - 2
                )
                ...
                assert len(input_ids) == len(
                    tmp
                ), "Span tracking tokenization produced inconsistent result!"
...
class BoolqProcessor(DataProcessor):
    ...
    def write_preds(self, preds, ex_ids, out_dir):
        ...
        idx2label = dict(enumerate(self.get_labels()))
...
class CbProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_jsonl(os.path.join(data_dir, "val.jsonl")), "dev_matched"
        )
    ...
    def write_preds(self, preds, ex_ids, out_dir):
        ...
        with open(os.path.join(out_dir, "MultiRC.jsonl"), "w") as pred_fh:
            for psg_id, qst2ans in psg2qst2ans.items():
                ...
                pred_fh.write(
                    f"{json.dumps({'idx': psg_id, 'passage': {'questions': psgs}})}\n"
                )
    ...
    def get_answers(self, data_dir, set_type):
        ...
        if self._answers is None or set_type not in self._answers:
            ...
            for line in data:
                ...
                ents = [
                    passage[ent_dict["start"] : ent_dict["end"] + 1]
                ...
                for question_dict in line["qas"]:
                    ...
                    answers = (
                        [a["text"] for a in question_dict["answers"]]
...
class WscProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for line in lines:
            ...
            examples.append(
                SpanClassificationExample(
                    guid=guid, text_a=text_a, spans_a=[span1, span2], label=label
                )
            )
        return examples
...
class DiagnosticBroadProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        raise AssertionError(
            "Diagnostic tasks only have test data! Call get_test_examples instead"
        )
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_jsonl(os.path.join(data_dir, "AX-b.jsonl")), "test"
        )
