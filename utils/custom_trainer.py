...
class SuperGlueTrainerBasic(Trainer):
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            if args.per_device_eval_batch_size == 1:
                if isinstance(inputs["guid"], torch.Tensor) and inputs["guid"].ndim == 2:
                    all_guids.append(inputs["guid"].squeeze(0).numpy().tolist())
                else:
                    all_guids.append(inputs["guid"])
            else:
...
class SuperGlueTrainer(Trainer):
    def make_grad_bank(self, model: PreTrainedModel):
