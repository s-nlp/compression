    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for field_name, field_pad_idx, field_seq_idx in self.fields_to_pad:
            ...
            if field_values[field_name]:
                field_value_arrays = [
                    torch.as_tensor(values) for values in field_values[field_name]
                ]
                assert len({arr.ndim for arr in field_value_arrays}) == 1
                ...
                padded_tensor = torch.full(
                    [batch_size] + max_dim_lengths, field_pad_idx, dtype=torch.long
                )
