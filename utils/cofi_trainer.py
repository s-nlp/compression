...
class EvalCounter:
    """
    A class to keep track of evaluation metrics and epochs.
    ...
    best_eval_score : float
        The highest evaluation score seen so far.
    near_sparsity_eval_times : int
        The number of times the sparsity level has been near.
    level_best_score : dict
        A dictionary with sparsity levels as keys and best evaluation scores as values.
    """
    ...
    def round_nearest(self, x: float, a: float) -> float:
        return round(round(x / a) * a, -int(math.floor(math.log10(a))))

    def update(self, epoch: int, global_step: int, eval_score: float) -> bool:
        """
        Updates the evaluation score, epoch, and global step.
        ...
        global_step : int
            The current step in the training process.
        eval_score : float
            The evaluation score for the current epoch.

        Returns:
        --------
        bool
            True if the evaluation score improved
        """
        best_so_far = False
...
class AdditionalArguments:
    test: bool = field(
        default=False,
        metadata={"help": "Testing additional arguments."},
    )

    ex_name: str = field(
        default="superglue_models/cofi_pruning",
        metadata={"help": "Name of experiment. Base directory of output dir."},
    )
    pruning_type: str = field(
        default="structured_heads+structured_mlp+hidden+layer",
        metadata={"help": "Type of pruning"},
    )
    reg_learning_rate: float = field(
        default=0.1, metadata={"help": "Learning rate for regularization."}
    )
    scheduler_type: str = field(default="linear", metadata={"help": "type of scheduler"})
    freeze_embeddings: bool = field(
        default=False, metadata={"help": "Whether we should freeze the embeddings."}
    )

    pretrained_pruned_model: str = field(
        default=None, metadata={"help": "Path of pretrained model."}
    )

    droprate_init: float = field(
        default=0.5, metadata={"help": "Init parameter for loga"}
    )
    temperature: float = field(
        default=2.0 / 3.0,
        metadata={"help": "Temperature controlling hard concrete distribution"},
    )
    prepruning_finetune_epochs: int = field(
        default=1, metadata={"help": "Finetuning epochs before pruning"}
    )
    lagrangian_warmup_epochs: int = field(
        default=2, metadata={"help": "Number of epochs for lagrangian warmup"}
    )
    target_sparsity: float = field(
        default=0.65, metadata={"help": "Target sparsity (pruned percentage)"}
    )
    sparsity_epsilon: float = field(default=0, metadata={"help": "Epsilon for sparsity"})

    # distillation setup
    distillation_path: str = field(
        default=None, metadata={"help": "Path of the teacher model for distillation."}
    )
    do_distill: bool = field(
        default=False,
        metadata={"help": "Whether to do distillation or not, prediction layer."},
    )
    do_layer_distill: bool = field(
        default=False, metadata={"help": "Align layer output through distillation"}
    )
    layer_distill_version: int = field(
        default=1,
        metadata={
            "help": "1: add loss to each layer, 2: add loss to existing layers only"
        },
    )
    distill_loss_alpha: float = field(
        default=0.9, metadata={"help": "Distillation loss weight"}
    )
    distill_ce_loss_alpha: float = field(
        default=0.1, metadata={"help": "Distillation cross entrypy loss weight"}
    )
    distill_temp: float = field(
        default=2.0 / 3.0, metadata={"help": "Distillation temperature"}
    )
...
class CoFiTrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.eval_counter = EvalCounter()
        self.start_saving_best = self.additional_args.pruning_type is None
        ...
        if self.args.eval_steps is None:
            self.args.eval_steps = math.ceil(
    ...
    def create_optimizer_and_scheduler(
        self, num_training_steps: int, build_l0_optimizer: bool = True
    ):
            ...
            main_model_params = [
                {
                    "params": [
                        ...
                        and not any(fk in n for fk in freeze_keywords)
                    ],
                ...
                l0_params = [
                    {
                        "params": [
                            p
                            for n, p in self.l0_module.named_parameters()
                            if "lambda" not in n
                        ],
                        "weight_decay": 0.0,
    ...
    def train(self):
        ...
        self.create_optimizer_and_scheduler(
            num_training_steps=self.t_total, build_l0_optimizer=self.start_prune
        )
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):  #! 20 epoch
            ...
            for step, inputs in enumerate(epoch_iterator):
                if (
                    ...
                    and self.global_step == self.prepruning_finetune_steps
                ):  # before pruning, run 12272 steps
    ...
    def prediction_loop(
        ...
        for ii, inputs in enumerate(
            tqdm(dataloader, desc=description, disable=disable_tqdm)
        ):
            if zs is not None:
                if ii == 0:
            ...
            if logits is not None:
                preds_host = (
                    logits if preds_host is None else nested_concat(preds_host, logits)
                )
            if labels is not None:
                labels_host = (
                    labels if labels_host is None else nested_concat(labels_host, labels)
                )
            if loss is not None:
        ...
        if zs is not None:
            ...
            if (not self.start_saving_best) and (
                expected_sparsity - self.additional_args.target_sparsity
                >= -self.additional_args.sparsity_epsilon
            ):
                self.start_saving_best = True
                logger.info(
                    f"Starting saving the best from epoch {int(self.epoch)} and step {self.global_step}"
                )
        ...
        return PredictionOutput(
            predictions=all_preds, label_ids=all_labels, metrics=metrics
        )
    ...
    #                 torch.save(self.l0_module, os.path.join(best_dir, "l0_module.pt"))
    #             logger.info(
    ...
    def calculate_layer_distillation_loss(self, teacher_outputs, student_outputs, zs):
        ...
        if self.additional_args.do_layer_distill:  # only do layer distill
            ...
            teacher_layer_output = teacher_outputs[2][
                1:
            ]  # hidden states, with a length of 12. Every has a shape of [32, 65, 768]
            student_layer_output = student_outputs[2][1:]

            # distilliting existing layers
            if self.additional_args.layer_distill_version == 2:
                for layer_num, (t_layer_o, s_layer_o) in enumerate(
                    zip(teacher_layer_output, student_layer_output)
                ):
            ...
            # distilling layers with a minimal distance
                ...
                if self.additional_args.layer_distill_version > 4:
                    ...
                    if self.additional_args.layer_distill_version == 5:
                        specified_teacher_layers = sorted(
                            random.sample(specified_teacher_layers, 4)
                        )
                        ...
                        for i in range(0, len(specified_teacher_layers), skip_window):
                            result_layers_T.append(
                                random.sample(
                                    specified_teacher_layers[i : i + skip_window], 1
                                )[0]
                            )
                ...
                transformed_s_layer_o = [
                    self.model.layer_transformation(s_layer_o)
                    for s_layer_o in student_layer_output
                ]
                specified_teacher_layer_reps = [
                    teacher_layer_output[i] for i in specified_teacher_layers
                ]  # teacher: 4x[32,113,768]

                device = transformed_s_layer_o[0].device
                for t_layer_o in specified_teacher_layer_reps:
                    for i, s_layer_o in enumerate(
                        transformed_s_layer_o
                    ):  # student: 12x[32,113,768]
                ...
                layer_loss += layerwiseloss[
                    layerwise, alignment
                ].sum()  # layerwise: teacher (specified layers) / alignment: student (min loss layers) / layerwiseloss: [4,12]
                if self.global_step % 100 == 0:
                    logger.info(
                        f"v{self.additional_args.layer_distill_version} Global step: {self.global_step}, Alignment: "
                        + str(alignment)
                    )
            return layer_loss
        else:
    ...
    def training_step(
        ...
        if self.teacher_model is not None:
            with torch.no_grad():
                ...
                teacher_inputs = {
                    key: inputs[key] for key in teacher_inputs_keys if key in inputs
                }
        ...
        return {
            ...
            "distill_layer_loss": distill_loss.detach()
            if distill_loss is not None
            else None,
            "distill_ce_loss": distill_ce_loss.detach()
            if distill_ce_loss is not None
            else None,
        }
