...
def synth_bench():
    ...
    parser.add_argument(
        "--model_name_or_path", default="bert-base-uncased", help="model from HF"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=(16, 128),
        help="sequence sizes for bench",
    )
    parser.add_argument(
        "--max_bench_iter",
        help="Seconds for warmup blocked_autorange benchmark",
        type=int,
        default=1,
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=(10, 10, 10),
        help="ranks of TT decomposition",
    )
    parser.add_argument(
        "--tt_input_dims",
        type=int,
    ...
    if args_bench.do_bench:
        ...
        args_full = PyTorchBenchmarkArguments(
            models=[args_bench.model_name_or_path],
            batch_sizes=args_bench.batch_sizes,
            sequence_lengths=args_bench.sequence_lengths,
            training=args_bench.bench_on_train,
            inference=args_bench.bench_on_eval,
            cuda=True,
            multi_process=True,
            verbose=False,
            trace_memory_line_by_line=False,
            repeat=args_bench.max_bench_iter,
            ...
            save_to_csv=True,
            env_print=False,
            exp_name=args_bench.comp_func,
            rank=args_bench.rank,
            tt_ranks=args_bench.tt_ranks,
            tt_input_dims=args_bench.tt_input_dims,
