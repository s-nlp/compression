...
def fake_train(model, d, batch_size, seq_length, device, n_steps=5, seed=0):
...
def run_benchmark(model, d, batch_size, seq_length, device):
    ...
    model = fake_train(model, d, batch_size, seq_length, device)
...
def main():
    ...
    print(f"Testing with batch size {batch_size}, d = {d}, n_blocks = {n_blocks}, ranks = {ranks}")
