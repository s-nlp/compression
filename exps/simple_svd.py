...
def w_svd_func_inv(model_to, svd_rank = 51, weight_int=None, weight_out=None, weight_count=None):
    for i in range(model_to.config.num_hidden_layers):
        ...
        bias = model_to.bert.encoder.layer[i].intermediate.dense.bias
        U, S, Vt = torch.linalg.svd((I @ model_to.bert.encoder.layer[i].intermediate.dense.weight.T).T , full_matrices=False)
    ...
    for i in range(model_to.config.num_hidden_layers):
        ...
        bias = model_to.bert.encoder.layer[i].output.dense.bias
        U, S, Vt = torch.linalg.svd((I @ model_to.bert.encoder.layer[i].output.dense.weight.T).T , full_matrices=False)
        w1 = torch.linalg.lstsq(I, torch.mm(torch.diag(torch.sqrt(S[0:svd_rank])),Vt[0:svd_rank, :]).T).solution.T
        w2 = torch.mm(U[:, 0:svd_rank], torch.diag(torch.sqrt(S[0:svd_rank])))
