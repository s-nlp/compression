...
def adaptive_svd(model_to, svd_ranks: List[int]):
    for i in range(model_to.config.num_hidden_layers):
        rank_i = svd_ranks[i]
        rank_o = svd_ranks[model_to.config.num_hidden_layers + i]
        bias = model_to.bert.encoder.layer[i].intermediate.dense.bias
        U, S, Vt = torch.linalg.svd(
            model_to.bert.encoder.layer[i].intermediate.dense.weight, full_matrices=False
        )
        # truncate SVD and fuse Sigma matrix
        w1 = torch.mm(torch.diag(torch.sqrt(S[:rank_i])), Vt[0:rank_i, :])
        w2 = torch.mm(U[:, 0:rank_i], torch.diag(torch.sqrt(S[:rank_i])))

        model_to.bert.encoder.layer[i].intermediate.dense = SVD_dec(
            model_to.config.hidden_size, model_to.config.intermediate_size, rank_i
        )
        ...
        # truncate SVD and fuse Sigma matrix
        w1 = torch.mm(torch.diag(torch.sqrt(S[:rank_o])), Vt[0:rank_o, :])
        w2 = torch.mm(U[:, 0:rank_o], torch.diag(torch.sqrt(S[:rank_o])))

        model_to.bert.encoder.layer[i].output.dense = SVD_dec(
            model_to.config.intermediate_size, model_to.config.hidden_size, rank_o
        )
