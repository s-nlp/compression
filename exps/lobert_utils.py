...
from .lobert_models import (
    BertEncoder_FALCON_4d,
    BertEncoder_FALCON_vertical,
    BertEncoder_ffn_tucker,
    BertEncoder_tucker_4d,
    BertEncoder_tucker_vertical,
    BertIntermediate_svd,
    BertOutput_svd,
    BertSelfAttention_FALCON_layer,
    BertSelfAttention_svd,
    BertSelfAttention_tucker_layer,
)
...
def lobert_student(
    ...
    for i in range(num_hidden_layers):
        intermediate_tensor.append(
            student_model.bert.encoder.layer[i].intermediate.dense.weight
        )
        output_tensor.append(student_model.bert.encoder.layer[i].output.dense.weight)

        layer_tensor = [
            student_model.bert.encoder.layer[i].attention.self.query.weight.data,
            student_model.bert.encoder.layer[i].attention.self.key.weight.data,
            student_model.bert.encoder.layer[i].attention.self.value.weight.data,
        ]
...
def decompose_ffn_svd(
    ...
    for i in range(config.num_hidden_layers):
        ...
        w1 = torch.nn.Parameter(
            torch.mm(torch.diag(torch.sqrt(S[0:svd_rank])), Vt[0:svd_rank, :])
        )
        w2 = torch.nn.Parameter(
            torch.mm(U[:, 0:svd_rank], torch.diag(torch.sqrt(S[0:svd_rank])))
        )
        ...
        intermediate.dense_v.bias.data.copy_(
            student_model.bert.encoder.layer[i].intermediate.dense.bias.data
        )
        ...
        output.dense_v.bias.data.copy_(
            student_model.bert.encoder.layer[i].output.dense.bias.data
        )
...
def decompose_ffn_tucker_vertical(
    student_model,
    intermediate_tensor,
    output_tensor,
    config,
    ...
    core_i, factors_i = tucker(
        intermediate_tensor, rank=[rank_fst, rank_snd * 4, rank_snd]
    )
    ...
    for i in range(config.num_hidden_layers):
        encoder.layer[i].intermediate.bias.data.copy_(
            student_model.bert.encoder.layer[i].intermediate.dense.bias.data
        )
        encoder.layer[i].output.bias.data.copy_(
            student_model.bert.encoder.layer[i].output.dense.bias.data
        )
...
def decompose_self_svd(
    student_model, self_attention_tensor, config, num_hidden_layers, svd_rank=192
):
    """
    ...
    for i in range(num_hidden_layers):
        ...
        attention_layer.query_v.bias.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.query.bias.data
        )

        attention_layer.key_u.weight.data.copy_(k1)
        attention_layer.key_v.weight.data.copy_(k2)
        attention_layer.key_v.bias.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.value.bias.data
        )

        attention_layer.value_u.weight.data.copy_(v1)
        attention_layer.value_v.weight.data.copy_(v2)
        attention_layer.value_v.bias.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.value.bias.data
        )
...
def decompose_self_tucker_layer(
    student_model,
    self_attention_tensor,
    config,
    num_hidden_layers,
    ...
    for i in range(config.num_hidden_layers):
        ...
        layer.bias_q.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.query.bias.data
        )
        layer.bias_k.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.key.bias.data
        )
        layer.bias_v.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.value.bias.data
        )
...
def decompose_self_falcon_layer(
    student_model, self_attention_tensor, config, num_hidden_layers
):
    """
...
def decompose_self_tucker_vertical(
    student_model,
    self_attention_tensor,
    config,
    num_hidden_layers,
    ...
    :return: student_model
    """
    self_attention_tensor = self_attention_tensor.permute(
        1, 0, 2, 3
    )  # shape = [3, 12, 768, 768]
    ...
    for i in range(config.num_hidden_layers):
        encoder.layer[i].attention.self.bias_q.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.query.bias.data
        )
        encoder.layer[i].attention.self.bias_k.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.key.bias.data
        )
        encoder.layer[i].attention.self.bias_v.data.copy_(
            student_model.bert.encoder.layer[i].attention.self.value.bias.data
        )
        ...
        encoder.layer[i].attention.output = student_model.bert.encoder.layer[
            i
        ].attention.output
...
def decompose_self_falcon_vertical(
    student_model, self_attention_tensor, config, num_hidden_layers
):
    """
...
def decompose_self_tucker_4d(
    ...
    core, factors = tucker(
        self_attention_tensor, rank=[rank_fst, rank_snd, rank_trd, rank_trd]
    )
