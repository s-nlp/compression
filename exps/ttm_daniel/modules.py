...
from .linalg import ttd, weighted_ttd, other_weighted_ttd
...
class TTCompressedLinear(CompressedLinear):
    ...
    def from_linear(cls, linear: T.nn.Linear,
        ...
        #print('cores before', [c.shape for c in cores])
        ...
        #print('cores after', [c.shape for c in cores])
...
class FWTTCompressedLinear(CompressedLinear):
    ...
    def from_linear(cls, linear: T.nn.Linear, fisher_information: T.Tensor,
        ...
        # Reshape TT-cores back to TT-matrix cores (TTM-cores).
        core_shapes = zip(tt_rank, *shape, tt_rank[1:])
        #print('FW shape', fisher_information.shape)
        #print('core_shapes',shape)
        #fisher_reshape = fisher_information.reshape(shape[0])
        #fisher_newshape = [fisher_reshape.mean(ii).reshape(1,-1,1,1).to(tensor.device) for ii in [(1,2),(0,2),(0,1)]]
        #print('FISHER', [f.shape for f in fisher_newshape])
        #cores = [core.reshape(core_shape) / FW
        #         for core, core_shape, FW in zip(cores, core_shapes, fisher_newshape)]
        cores = [core.reshape(core_shape)
                 for core, core_shape in zip(cores, core_shapes)]
...
class InvasiveFWTTCompressedLinear(CompressedLinear):
    ...
    def from_linear(cls, linear: T.nn.Linear, fisher_information: T.Tensor,
        ...
        return InvasiveFWTTCompressedLinear(cores, bias)
        #return TTCompressedLinear(cores, bias)